...
export function isUserLoggedIn(): boolean {
  ...
  // Пользователи, зарегистрированные до появления сессии
  return !!getUserName();
}
...
/** Регистрация нового аккаунта: сбрасывает старый профиль, если email другой. */
export function registerNewUser(name: string, email: string): void {
  if (typeof window === "undefined") return;
  const trimmedEmail = email.trim();
  const prevEmail = getSessionEmail();
  if (prevEmail && prevEmail !== trimmedEmail) {
    clearPersonalData();
  }
  clearHealthProfile();
  saveUserName(name);
